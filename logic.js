...
function startSurvey() {
    ...
    shuffledQuestions.forEach((question, index) => {
        ...
        const starRatingWrapper = document.createElement('div');
        starRatingWrapper.className = 'star-rating-wrapper';
        
        // "전혀 그렇지 않다" 라벨
        const leftLabel = document.createElement('span');
        leftLabel.className = 'scale-label';
        leftLabel.textContent = '전혀 그렇지 않다';
        ...
        rightLabel.textContent = '매우 그렇다';
        
        // 별점 생성 (5점부터 1점까지 역순으로)
        [5, 4, 3, 2, 1].forEach(value => {
            const input = document.createElement('input');
            input.type = 'radio';
            input.id = `star${value}_q${index}`;
            input.name = `q${index}`;
            input.value = value;
            
            const label = document.createElement('label');
            label.htmlFor = `star${value}_q${index}`;
            
            starRating.appendChild(input);
            starRating.appendChild(label);
        });
        
        starRatingWrapper.appendChild(leftLabel);
        starRatingWrapper.appendChild(starRating);
        starRatingWrapper.appendChild(rightLabel);
        ratingContainer.appendChild(starRatingWrapper);
